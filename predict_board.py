...
def test_predict_board(obtain_piece_probs_for_all_64_squares):
    ...
    for idx, jpg_file in enumerate(jpg_files):
        a1_square = a1_squares[idx % len(a1_squares)]  # Cycle through a1_squares if fewer than images
        previous_fen = previous_fens[idx % len(previous_fens)]  # Cycle through previous_fens similarly
        ...
        print_fen_comparison(
            ...
            fen,
            fens[idx % len(fens)],  # Cycle through fens if fewer than images
            used_previous_fen=(previous_fen is not None)
        )

        if previous_fen is not None and not check_validity_of_fen(previous_fen):
            print(
                f"Warning: the previous FEN for {os.path.basename(jpg_file)} is ignored "
                "because it is invalid for a standard physical chess set\n"
            )
            previous_fen = None

        # Re-run prediction with updated previous FEN if necessary
